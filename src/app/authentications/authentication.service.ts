...
export class AuthService {
    ...
    authUrl: string = "http://localhost:8080/api_v1/users/";
    authToken: string = "http://localhost:8080/api/authentication/client_token";
    constructor(
        ...
        this.addresses = afs.collection('addresses');
    ...
    async getClientToken(){
        ...
        let accounts:any = await new Promise((resolve, reject)=>{
            this.afs.collection('accounts', (ref => ref.where('uid', '==', currentuser.user_id))).snapshotChanges()
            .map((snapshot)=>{
        ...
        let address = await new Promise((resolve, reject)=>{
            this.afs.collection('addresses', (ref => ref.where('uid', '==', currentuser.user_id))).valueChanges().subscribe((addresses)=>{  
                resolve(addresses[0]);
            }, (err => {
                console.log(err)
                reject(err);
        ...
        let user =  {
            ...
            account_no: accounts.account_no || null,
    ...
    getUserAddresses(uid){
        return this.afs.collection('addresses', (ref => ref.where('uid', '==', uid))).valueChanges();
    }
    saveUserAddresses(addr){
