...
export class RegisterComponent implements OnInit {
  ...
  registerUser(customer){
    ...
    let phone = Number(customer.telephone.mobile);
    let present = Date.now();
    let acNum = Number(phone + present);
    // console.log(acNum);
    let users = {
    ...
    // Creating user account after setup
    this.authService.createUser(customER).subscribe((user:any)=>{
      ...
      if(user.addressStatus == "addressSuccess"){
        // send to ui
      }
      console.log(user);
