...
export class AdvertService {
    ...
    advertCached$;
    constructor(
      private _http:Http, private afs: AngularFirestore
    ){
    ...
    getAdvertCached(){
     ...
     handleError(err):Observable<any>{
      ...
      getShopAdverts(section, tag){
        return this.advertisement = this.afs.collection('adverts', ref => ref.where('section', '==', section).where('tag', '==', tag)).valueChanges();
      }
}